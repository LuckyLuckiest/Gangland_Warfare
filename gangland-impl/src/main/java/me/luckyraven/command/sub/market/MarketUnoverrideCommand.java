...
class MarketUnoverrideCommand extends SubArgument {
	...
	MarketUnoverrideCommand(Gangland gangland, Tree<Argument> tree, Argument parent,
	                        MarketItemRegistry itemRegistry, MarketMessageContract messages) {
		super(gangland, new String[]{"unoverride", "clearoverride"}, tree, parent);
	...
	private OptionalArgument itemArg() {
		return new OptionalArgument(gangland, tree, (argument, sender, args) -> {
			...
			itemRegistry.find(id).ifPresentOrElse(state -> {
				state.setOverridePrice(null);
				itemRegistry.persist(state);
				sender.sendMessage(messages.overrideCleared(id));
