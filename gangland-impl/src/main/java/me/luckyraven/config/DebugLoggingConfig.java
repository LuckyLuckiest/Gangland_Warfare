 ...
 *
 * <p>Depends on {@link Settings} so the topological sort places this bean after the settings file has finished
 * loading. The {@code Settings} parameter is intentionally unused in the method body; its only purpose is to establish
 * the dependency edge.
 *
...
public class DebugLoggingConfig {
	...
	private static List<String> discoverAllModules() {
		...
		try {
			ClassLoader      classLoader = DebugLoggingConfig.class.getClassLoader();
	...
	public DebugLoggingInitializer debugLoggingInitializer(Settings settings) {
		if (!Settings.isDebugEnabled()) {
			return new DebugLoggingInitializer();
		}
	...
	public static final class DebugLoggingInitializer {
	}
}
