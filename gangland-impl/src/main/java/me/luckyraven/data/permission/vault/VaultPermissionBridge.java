...
/**
 * Static facade over Vault's {@link Permission} service. Mirrors the cadence of
 * {@link me.luckyraven.economy.bank.EconomyHandler#setVaultEconomy}: the bridge is plugged in from
 * {@code Gangland.onEnable} once a Vault {@code Permission} provider is registered, and cleared on {@code onDisable}.
 * Every public method is a safe no-op when the provider is absent, so callers never need to guard.
 *
...
public final class VaultPermissionBridge {
	...
	public static void set(@Nullable Permission provider) {
		vault = provider;
	}
	...
	public static void addToGroup(@Nullable OfflinePlayer player, @Nullable String group) {
		...
		try {
			...
			log.warn("Vault playerAddGroup failed for {} / {}: {}", player.getName(), group, exception.getMessage());
		}
	}
