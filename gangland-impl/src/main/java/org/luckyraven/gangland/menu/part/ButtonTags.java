...
public record ButtonTags(String previousPage, String homePage, String nextPage) {
	...
	/**
	 * Replaces the deleted {@code settings.yml} {@code Inventory.Multi_Inventory} block (CUT gate — the block and
	 * its {@code Settings} getters were deleted outright, per the plan, rather than migrated). These are the exact
	 * base64 head textures that block always shipped, so paginated nav buttons render identically to before; only
	 * the YAML configurability is lost.
	 */
	public static final ButtonTags DEFAULT = new ButtonTags(
