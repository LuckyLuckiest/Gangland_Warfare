 ...
 * fallback (the pristine jar resource). A key missing from the primary falls through to the fallback, so admin typos or
 * accidental deletions degrade to the default string instead of surfacing as {@code null} at call sites. Constructed by
 * {@link LanguageLoader} after the per-language {@code message_<lang>.yml} is loaded.
 */
