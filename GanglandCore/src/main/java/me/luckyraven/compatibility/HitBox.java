...
import me.luckyraven.util.ray.EntityTraceResult;
import me.luckyraven.util.ray.RayTraceResult;
...
public class HitBox {
	...
	@Getter private double       minX, minY, minZ;
	@Getter private double maxX, maxY, maxZ;
	...
	public HitBox grow(double amount) {
		...
		return this.modify(minX - amount, minY - amount, minZ - amount, maxX + amount, maxY + amount, maxZ + amount);
	}
	...
	public HitBox grow(double width, double height) {
		width /= 2;
		return this.modify(minX - width, minY, minZ - width, maxX + width, maxY + height, maxZ + width);
	}

	/**
	 * Uses BoundingBox class method expand(Vector, double). Easier backwards compatibility this way. <a
	 * href="https://hub.spigotmc.org/javadocs/bukkit/org/bukkit/util/BoundingBox.html#expand(org.bukkit.util.Vector,double)">...</a>
	 *
	...
	/**
	 * Uses BoundingBox class method expand(double, double, double, double, double, double). Easier backwards
	 * compatibility this way. <a
	 * href="https://hub.spigotmc.org/javadocs/bukkit/org/bukkit/util/BoundingBox.html#expand(double,double,double,double,double,double)">...</a>
	 *
	 * @return this hit box with expansion
	 */
	public HitBox expand(double negativeX, double negativeY, double negativeZ, double positiveX, double positiveY,
						 double positiveZ) {
	...
	public void outlineAllBoxes(Entity player) {
		if (voxelShape != null) {
			...
			for (HitBox voxel : voxelShape) {
				voxel.outlineMainBox(player, colors[i]);
				if (++i >= colors.length) {
					i = 1;
				}
			}
	...
	/**
	 * Uses BoundingBox class method rayTrace(Vector, Vector, double) with slight modifications. Easier backwards
	 * compatibility this way. <a
	 * href="https://hub.spigotmc.org/javadocs/bukkit/org/bukkit/util/BoundingBox.html#rayTrace(org.bukkit.util.Vector,org.bukkit.util.Vector,double)">...</a>
	 */
	private RayTraceResult ray(Vector location, Vector normalizedMotion) {
		...
		if (block != null) {
			return new BlockTraceResult(location, normalizedMotion, this, hitBlockFaceMin, hitBlockFaceMax, tMin, tMax,
										block);
		...
		return new EntityTraceResult(location, normalizedMotion, this, hitBlockFaceMin, hitBlockFaceMax, tMin, tMax,
									 livingEntity);
	}
}
