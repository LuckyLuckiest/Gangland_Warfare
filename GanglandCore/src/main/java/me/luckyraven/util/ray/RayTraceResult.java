...
public class RayTraceResult {
	...
	@Getter private final  double    hitMin;
	@Getter private final  double    hitMax;

	// cached if calculated
	private @Nullable Vector hitLocation;
	private @Nullable Vector exitLocation;

	public RayTraceResult(@NotNull Vector origin, @NotNull Vector direction, @NotNull HitBox hitBox,
						  @NotNull BlockFace hitFace, @NotNull BlockFace exitFace, double hitMin, double hitMax) {
		this.origin    = origin;
	...
	/**
	 * Returns the exact coordinates hit, in world space.
	 *
	...
	public double getHitMinClamped() {
		return Math.max(hitMin, 0D); // assert positive number
	}

	/**
	 * Returns the distance traveled through the hitbox. For a block, this number will not exceed <code>sqrt(2)</code>.
	 * This method will always return a positive number.
	 *
	 * @return The distance traveled through the hitbox.
	 */
	...
	public String toString() {
		return "RayTraceResult{" + "hitFace=" + hitFace + ", hitLocation=" + hitLocation + ", hitMin=" + hitMin +
			   ", hitMax=" + hitMax + '}';
	}
}