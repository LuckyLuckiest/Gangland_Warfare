package org.luckyraven.gangland.weapon.sign.view;
...
/**
 * The {@code view} sign's weapon, ammunition and wearable branches, moved out of the core's
 * {@code ViewInventoryAspect} verbatim when the sign extension seam was introduced. Tries weapon, then
 * ammunition, then wearable, returning {@code true} on the first hit — preserving the chain position the core's
 * inline branches used to occupy (module providers run before the core's generic fallback).
 */
public class WeaponSignViewProvider implements SignViewProvider {
	...
	public WeaponSignViewProvider(JavaPlugin plugin, WeaponService weaponService, AmmunitionManager ammunitionManager,
	                              WearableService wearableService, UniqueItemAddon uniqueItemAddon) {
		this.plugin            = plugin;
	...
	public boolean open(Player player, String content) {
		Weapon weapon = findWeapon(content);
		if (weapon != null) {
			openWeaponView(player, weapon);
			return true;
		}

		if (ammunitionManager.getAmmunitionKeys().contains(content)) {
			openAmmunitionView(player, content);
			return true;
		}

		Wearable wearable = wearableService.getWearable(content);
		if (wearable != null) {
