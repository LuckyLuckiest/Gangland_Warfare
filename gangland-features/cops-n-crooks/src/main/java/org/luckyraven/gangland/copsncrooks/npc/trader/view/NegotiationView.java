 ...
 * {@link QuantitySelectorView} via {@link MultiPanelInventory#switchTo(String)} — both are panels in the trader flow,
 * so the transitions are same-handle re-renders (no inventory swap) where size + title permit.
 */
