 ...
 * {@link me.luckyraven.copsncrooks.npc.civilian.npc.CivilianNpc} of the configured type — the civilian system handles
 * AI / health / model / equipment entirely. This manager just owns the spawn / lookup / engage / disengage lifecycle
 * and tags the underlying Citizens NPC with the turf-id metadata so the right-click listener can route clicks back into
 * the panel flow.
 */
@CustomLog
public final class TurfPowerupManager implements BeanLifecycle {
	...
	/**
	 * Called by {@link me.luckyraven.copsncrooks.listener.turf.TurfPowerupChunkLoadListener} when Bukkit loads a chunk.
	 * Scans the pending-spawn queue for any Quartermaster whose anchor sits in that chunk and spawns it now. Once
	 * spawned, the entry leaves the pending queue.
	 */
