...
class GangColorCommand extends SubArgument {
	...
	// class is the impl-only YAML menu dialect (not visible to a module, W51); these 3 style defaults are the
	// only thing this command ever needed from it, so they're inlined rather than adding api surface for them.
	private static final String DEFAULT_FILL_ITEM = "BLACK_STAINED_GLASS_PANE";
	private static final String DEFAULT_FILL_NAME = " ";
	...
	private void displayColors(Player player) {
		...
		for (Color color : Color.values()) {
			...
			colorMenu.slot(slot, ItemComponent.of(previewItem).onAnyClick(ctx -> {
				...
				confirmMenu.fill(FillComponent.of(materialOf(DEFAULT_FILL_ITEM))
				                              .name(DEFAULT_FILL_NAME));
		...
		// carve-out).
		colorMenu.border(BorderComponent.of(materialOf(DEFAULT_LINE_NAME))
		                                .name(DEFAULT_FILL_NAME));
