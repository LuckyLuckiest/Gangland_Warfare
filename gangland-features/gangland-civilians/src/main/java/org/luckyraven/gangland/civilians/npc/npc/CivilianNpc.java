...
public class CivilianNpc extends AbstractNpc {
	...
	public CivilianNpc(JavaPlugin plugin, NPC npc, CivilianTypeConfig typeConfig, @Nullable String groupId,
		...
		super(plugin, npc, spawnLocation, navConfig, typeConfig.ai().difficulty());
	...
	public void equip() {
		...
		// AbstractNpc#refreshHeldItem — this vanilla-material pool is only the fallback for a civilian with no
		// ranged attack assigned (no weapon name pool, or Bartizan absent).
		if (!typeConfig.weaponPool().isEmpty()) {
