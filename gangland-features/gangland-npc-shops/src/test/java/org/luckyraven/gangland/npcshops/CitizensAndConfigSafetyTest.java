 ...
 * pass — {@code BankerInteractListener} and {@code TraderInteractListener} both declared an {@code @EventHandler}
 * parameter of Citizens' {@code NPCRightClickEvent} with no {@code condition = "isCitizensAvailable"} gate. Pins the
 * requirement going forward: no {@code @Configuration}/{@code @ListenerHandler}/{@code @CommandHandler}/
 * {@code @Repository} class in this module may carry a Citizens type in any of its own declared method/constructor
 * signatures, and no {@code @Configuration} class may ask for a dependency that is not yet in the container when
 * Keystone instantiates it.
 */
class CitizensAndConfigSafetyTest {

	private static final String BASE_PACKAGE = "org.luckyraven.gangland.npcshops";

	@Test
	@DisplayName("no scanned npc-shops class exposes a Citizens type in its own signature")
