 ...
 * sides of the item pipeline: {@link org.luckyraven.keystone.item.ItemConverter}s (string → ItemStack) are registered
 * under the kind's label, and {@link org.luckyraven.keystone.item.ItemSerializer}s (ItemStack → string) expose the
 * kind via {@code ItemSerializer#kind()}. Renaming a label here updates every converter, serializer, and ledger id at
 * once — callers never hardcode the string.
 *
